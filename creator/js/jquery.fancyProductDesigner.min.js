...
var FancyProductDesigner = function (elem, opts) {
    ...
    //load sidebar html
    $.post(options.templatesDirectory + 'productdesigner.html',
        options.labels,
        function (html) {
    ...
    var _initBars = function () {
        ...
        // HB Holger Brandt CUSTOM
        // July 2015
        $('#fpd').after($contextDialog);
        $('#fpd').css('float', 'right');
        $contextDialog.removeClass('fpd-fixed');
        $contextDialog.css({
            'display': 'none',
            'box-shadow': 'none'/*
            'width': '460px'*/
        ...
        $contextDialog.find('nav.fpd-dialog-head').css({'height': '64px'});
        $contextDialog.find('nav.fpd-dialog-head *').css({'line-height': '64px'});
        $contextDialog.find('nav.fpd-dialog-head i.fpd-icon-drag').css({'display': 'none'});
        $contextDialog.find('nav.fpd-dialog-head div.fpd-close-dialog').css({'display': 'none'});
        $contextDialog.find('nav.fpd-dialog-head').css({'cursor': 'default' });
        $('div.fpd-btn[data-context="products"]').trigger('mouseup');
        $contextDialog.show();
        setTimeout(function() {

            /*$contextDialog[0].style.setProperty('float', 'left', 'important');
            $contextDialog[0].style.setProperty('position', 'static', 'important');*/
        ...
        stage.on({
            'mouse:down': function (opts) {
                ...
                if (opts.target == undefined) {
                    ...
                    thisClass.deselectElement();
                    // HB Holger Brandt CUSTOM Show "Add element" context dialog when user clicks in empty area deselecting all
                    $('div.fpd-btn[data-context="adds"]').trigger('mouseup');
                }
                else {
                    ...
                    if (targetCorner == 'tl' && currentElement.params.isInitial !== true && !currentElement.params.hasUploadZone) {
                        var newParams = jQuery.extend({}, currentElement.params, {
                            autoSelect: true,
                            x: currentElement.params.x + currentElement.width*parseFloat(currentElement.params.scale)
                        });
                        newParams.topped = true;
            ...
            'object:rotating': function (opts) {
                if ((currentElement && currentElement.params.rotatable) || options.editorMode) {
                    ...
                    currentElement.set({
                        originX: currentElement.params.originX,
                        originY: currentElement.params.originY
                    });
    ...
    FancyProductDesigner.prototype.getProduct = function (onlyEditableElements) {
        ...
        for (var i = 0; i < objects.length; ++i) {
            ...
            var jsonItem = {
                ...
                type: (object.type == 'i-text' || object.type == 'curvedText') ? 'text' : 'image'
            };
    ...
    FancyProductDesigner.prototype.getViewsDataURL = function (format, backgroundColor, multiplier) {
        ...
        }).setBackgroundColor(backgroundColor, function () {
    ...
    FancyProductDesigner.prototype.getScalingByDimesions = function (imgW, imgH, resizeToW, resizeToH) {
        ...
        if (imgW > imgH) {
            if (imgW > resizeToW) {
                scaling = resizeToW / imgW;
            }
            if (scaling * imgH > resizeToH) {
                scaling = resizeToH / imgH;
            }
        }
        else {
            if (imgH > resizeToH) {
    ...
    FancyProductDesigner.prototype.selectProduct = function (index) {
        ...
        else if (index > thisClass.getProductsLength() - 1) {
            currentProductIndex = thisClass.getProductsLength() - 1;
        }
    ...
    FancyProductDesigner.prototype.selectView = function (index) {
        ...
        else if (index > $viewSelection.children().size() - 1) {
            currentViewIndex = $viewSelection.children().size() - 1;
        }
        ...
        // HB Holger Brandt CUSTOM Show "Add element" context dialog when choosing other view (e.g. changing from front to back view)
        $('div.fpd-btn[data-context="adds"]').trigger('mouseup');
    ...
    FancyProductDesigner.prototype.removeProduct = function (index) {
        ...
        if (index < 0) {
            ...
            index = thisClass.getProductsLength() - 1;
        }
    ...
    FancyProductDesigner.prototype.addDesign = function (source, title, parameters, category, thumbnail) {
        ...
        //categories are used
            ...
            designCategories[category].push({
                source: source,
                title: title,
...
(function ($) {
    ...
    jQuery.fn.fancyProductDesigner = function (options) {
        ...
        return this.each(function () {
            ...
            if (element.data('fancy-product-designer')) {
                return
            }
            ;
    ...
    $.fn.fancyProductDesigner.defaults = {
        ...
        lazyLoad: true,
        dialogBoxPositioning: 'left', //dynamic, left or right
        elementParameters: {
            x: 0, //the x-position
            y: 0, //the y-position
            z: -1, //Allows to set the z-index of an element, -1 means it will be added on the stack of layers
